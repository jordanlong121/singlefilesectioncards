...
t("background adjust layers: black under 100, white over, empty at 100", () => {
  assert.equal(backgroundLightLayer(100), "");
  assert.equal(backgroundLightLayer(60), "rgba(0, 0, 0, 0.4)");
...
t("deckExcerpt: frontmatter dropped, markers stripped, capped with an ellipsis", () => {
  const excerpt = deckExcerpt(
    ["---", "tag: x", "---", "", "## Heading", "> quoted line", "- [x] a done task", "1. ordered", "plain text"].join("\n"),
  );
  assert.equal(excerpt, "Heading\nquoted line\na done task\nordered\nplain text");
  const long = deckExcerpt("x".repeat(500), 8, 40);
  assert.equal(long.length, 40);
  assert.ok(long.endsWith("…"));
  assert.equal(deckExcerpt(""), "");
});
...
t("plannerCards: one loose card above the first heading, then a subcard per heading; deeper headings stay inside", () => {
  const body = L("\n- [ ] loose\n- [ ] also loose\n\n#### Morning\n- [ ] run\n##### detail\ntext\n\n#### Afternoon\n- [ ] dentist");
...
t("plannerCards: lines after the first sub-heading never form a loose card", () => {
  const cards = plannerCards(L("#### A\n- [ ] inside a\n#### B\n- [ ] inside b"));
  assert.deepEqual(cards.map((c) => c.kind), ["sub", "sub"]);
});

t("wholeNoteSection: the note below its properties, headless, spanning to the end", () => {
  const lines = L("---\ntitle: x\n---\n\n# Sept\n### 2026-09-14\n- [ ] a\n# Aug\n### 2026-08-31\n- [ ] b");
  const s = wholeNoteSection(lines);
  assert.equal(s.startLine, 4);
...
t("bareMonthIndex: a lone month name, long or short; anything else is 0", () => {
  assert.equal(bareMonthIndex("August"), 8);
  assert.equal(bareMonthIndex(" aug "), 8);
...
t("saved layouts: equality ignores savedAt and treats a missing zoom as 100%; any drift counts", () => {
  const a = { customGrid: { "## A": { x: 24, y: 24, w: 288, h: 192 } }, backgroundImage: "b.png", savedAt: 1 };
  assert.ok(canvasLayoutEquals(a, { ...a, savedAt: 2 }));
  assert.ok(canvasLayoutEquals(a, { ...a, customZoom: 1 }));
  assert.ok(!canvasLayoutEquals(a, { ...a, customZoom: 1.5 }));
  assert.ok(!canvasLayoutEquals(a, { ...a, customGrid: { "## A": { x: 48, y: 24, w: 288, h: 192 } } }));
  assert.ok(!canvasLayoutEquals(a, { ...a, customGrid: { ...a.customGrid, "## B": { x: 0, y: 0, w: 192, h: 192 } } }));
  assert.ok(!canvasLayoutEquals(a, { customGrid: a.customGrid, savedAt: 1 }), "a background removed is a change");
